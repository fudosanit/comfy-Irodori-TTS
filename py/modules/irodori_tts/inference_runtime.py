...
def _request_comfy_free_memory(memory_required: int, device: torch.device) -> None:
    """Ask ComfyUI to free VRAM before we load the TTS model (no-op outside Comfy)."""
    if device.type != "cuda" or memory_required <= 0:
        return
    try:
...
class RuntimeKey:
    ...
    # ComfyUI wrapper compatibility (unused by v4 load path, kept for cache-key parity)
...
class SamplingRequest:
    ...
    # ComfyUI wrapper compatibility (LoRA-by-stack not wired into v4 path yet)
...
class InferenceRuntime:
    ...
    def offload(self) -> None:
        """Move the model + codec to CPU to free VRAM between generations (ComfyUI)."""
    ...
    def _ensure_loaded(self) -> None:
        """Bring the model/codec back to their runtime devices after an offload()."""
    ...
    def synthesize(
        ...
        *,
        ...
        # ComfyUI wrapper passes a progress_callback; the v4 euler sampler has no
        # per-step hook, so we surface coarse start/end progress and keep the node working.
        self._ensure_loaded()
        if progress_callback is not None:
            try:
                progress_callback(0, int(req.num_steps))
            except Exception:
                pass
...
def offload_cached_runtime() -> None:
    """Offload (move to CPU) the cached runtime without discarding it (ComfyUI cache policy)."""
    with _RUNTIME_CACHE_LOCK:
        runtime = _RUNTIME_CACHE_VALUE
